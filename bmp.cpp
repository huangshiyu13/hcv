...
void Bmp::Initialize( int H , int W ) {
	...
	ima = new IMAGEDATA*[H];
	thisColor = new Color*[H];
	
	for ( int i = 0 ; i < H ; i++ ){
		ima[i] = new IMAGEDATA[W];
		thisColor[i] = new Color[W]; 
	}
...
void Bmp::Input( std::string file ) {
	...
	fread( &strHead , 1 , sizeof( myBITMAPFILEHEADER ) , fpi );
	fread( &strInfo , 1 , sizeof( myBITMAPINFOHEADER ) , fpi );
...
void Bmp::Output( std::string file ) {
	...
	fwrite( &strHead , 1 , sizeof( myBITMAPFILEHEADER ) , fpw );
	fwrite( &strInfo , 1 , sizeof( myBITMAPINFOHEADER ) , fpw );
...
void Bmp::SetColor( int i , int j , Color col ) {
	if (i >= GetH() || j >= GetW() || i < 0 || j < 0 ) return; 
	thisColor[i][j] = col;
...
void Bmp::SetColor( Vector2 O , Color col ) {
	int i = O.x;
	int j = O.y;
	SetColor(i,j,col); 
}
